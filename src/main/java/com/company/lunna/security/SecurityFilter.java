...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(login != null){
            Responsavel responsavel = responsavelRepository.findByEmailResp(login).orElseThrow(() -> new ResponsavelNotFoundException("Responsavel não encontrado"));
            var authorities = Collections.singletonList(new SimpleGrantedAuthority("ROLE_USER"));
            var authentication = new UsernamePasswordAuthenticationToken(responsavel, null, authorities);
